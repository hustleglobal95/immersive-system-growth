...
test("diagram rejects dangling edges",()=>{const broken=structuredClone(raw);const carrier=broken.scenes.find((scene)=>scene.diagram);assert.ok(carrier?.diagram);carrier.diagram.edges.push({from:"missing",to:"court"});assert.throws(()=>parseCinematicSystems(broken));});
