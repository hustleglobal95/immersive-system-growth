...
test("diagram reveal progresses without mutating geometry",()=>{const scene=parseCinematicSystems(raw).scenes.find(item=>item.id==="studio");assert.ok(scene?.diagram);const diagram=scene.diagram;const early=sampleDiagram(diagram,.2),late=sampleDiagram(diagram,.8);assert.equal(early.points.length,late.points.length);assert.ok(late.draw>early.draw);});
test("diagram rejects dangling edges",()=>{const broken=structuredClone(raw);const studio=broken.scenes.find((scene)=>scene.id==="studio");assert.ok(studio?.diagram);studio.diagram.edges.push({from:"missing",to:"court"});assert.throws(()=>parseCinematicSystems(broken));});
