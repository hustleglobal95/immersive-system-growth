...
export async function GET(request: Request) {
  try {
    ...
    if (!provider || !["meshy", "higgsfield-image", "higgsfield-video"].includes(provider)) throw new Error("Unknown generated asset provider.");
    ...
    try {
      ...
      const contentType = response.headers.get("content-type") ?? contentTypeFor(kind);
