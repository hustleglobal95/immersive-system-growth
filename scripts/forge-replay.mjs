...
const initialRevision = options["initial-revision"] === undefined ? 0 : Number(options["initial-revision"]);

if (!journalPath) {
  console.error("Usage: npm run forge:replay -- --journal <journal.json> [--input config/experience.json] [--initial-revision 0] [--output replayed-experience.json]");
  process.exit(2);
}
...
const initial = parseExperience(JSON.parse(await fs.readFile(inputPath, "utf8")));
