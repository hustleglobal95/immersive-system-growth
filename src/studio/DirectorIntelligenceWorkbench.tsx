...
export function DirectorIntelligenceWorkbench() {
  ...
  return <section className="director-intelligence">
    ...
    <div className="director-intelligence__stage">
      ...
      {tab === "memory" && <Panel title="Precedent, portfolio & taste memory"><div className="director-intelligence__cards">{report.precedents.map((item) => <article key={item.precedent.id}><span>{item.precedent.mediums.join(" · ")}</span><h4>{item.precedent.title}</h4><p>{item.precedent.transferableLessons[0]}</p><small>Transfer score {Math.round(item.score * 100)}%</small></article>)}</div><List title="Portfolio collisions" items={report.collisions.length ? report.collisions.slice(0,5).map((item) => `${item.projectId}: ${item.dimensions.overall}% — ${item.verdict} — ${item.reason}`) : ["No prior portfolio fingerprints supplied for this session."]} /><List title={`Creative-memory repetition — ${result.creativeMemory.verdict}`} items={result.creativeMemory.antiRepeat.length ? result.creativeMemory.antiRepeat : ["No material house-style repetition detected in supplied creative memory."]} /><List title="Layered taste" items={[result.tasteModel.rule,...result.tasteModel.contributions.map((item)=>`${item.layer}: weight ${item.weight.toFixed(2)} · confidence ${Math.round(item.confidence*100)}%`),`Selected territory influence ${result.tasteCalibration.adjustment.toFixed(2)} (capped ±0.50).`]} /></Panel>}
