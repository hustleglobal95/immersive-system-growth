...
export function LoopEnginePanel({
  ...
  const ready=selected.executable && proposalBound && vaultConfigured===true && vaultProject?.status==="active" && remoteRunnerEnabled===true && criticConnected===true && sourceReady;
  const runLabel=!proposalBound ? "Start from a proposal"
    ...
    : proposalBound && vaultProject && vaultSnapshot===undefined ? "Checking current checkpoint…"
      : proposalBound && vaultProject && !vaultMatchesWorking ? "Save current checkpoint first"
        : criticConnected===false ? "Visual critic not configured"
