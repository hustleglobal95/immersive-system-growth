...
const eventTypes = ["scene-enter", "scene-exit", "click", "hover-enter", "hover-leave", "pointer", "custom", "hotspot-open", "hotspot-close", "idle"] as const;
const actionTypes = ["set-variable", "set-state", "hotspot", "quality", "motion", "class", "seek", "emit", "sequence", "camera", "audio", "shader"] as const;
...
function NodeInspector({ node, graph, update }: { node: InteractionNode; graph: InteractionGraph; update: (node: InteractionNode) => void }) {
  ...
  if (node.kind === "trigger") return <>{common}<label>Event<select aria-label="Interaction trigger event" value={node.event} onChange={(event) => update(normalizeTrigger({ ...node, event: event.target.value as InteractionTriggerNode["event"] }))}>{eventTypes.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>{["click", "hover-enter", "hover-leave", "pointer", "hotspot-open", "hotspot-close"].includes(node.event) && <label>Target<input aria-label="Interaction trigger target" value={node.target ?? ""} onChange={(event) => update({ ...node, target: slugToken(event.target.value) || undefined })} /></label>}{["scene-enter", "scene-exit"].includes(node.event) && <label>Scene ID<input aria-label="Interaction trigger scene" value={node.sceneId ?? ""} onChange={(event) => update({ ...node, sceneId: slug(event.target.value) || undefined })} /></label>}{node.event === "custom" && <label>Custom event name<input aria-label="Interaction trigger name" value={node.name ?? ""} onChange={(event) => update({ ...node, name: slugToken(event.target.value) || undefined })} /></label>}{node.event === "idle" && <label>Idle delay ms<input aria-label="Interaction idle delay" type="number" min="250" max="120000" value={node.delayMs ?? 5000} onChange={(event) => update({ ...node, delayMs: Number(event.target.value) })} /></label>}<label>Allowed states<input aria-label="Interaction allowed states" value={node.states.join(", ")} onChange={(event) => update({ ...node, states: event.target.value.split(",").map(slug).filter((value) => graph.states.includes(value)) })} /></label></>;
...
function ActionEditor({ node, graph, update }: { node: Extract<InteractionNode, { kind: "action" }>; graph: InteractionGraph; update: (node: InteractionNode) => void }) {
  ...
  return <>
    ...
    {action.type === "sequence" && <NamedCommand action={action} label="Sequence" update={setAction} />}
    {action.type === "camera" && <NamedCommand action={action} label="Camera shot" update={setAction} />}
    {action.type === "audio" && <><NamedCommand action={action} label="Audio cue" update={setAction} /><label>Volume<input aria-label="Audio action volume" type="number" min="0" max="1" step="0.05" value={action.volume ?? 1} onChange={(event) => setAction({ ...action, volume: Math.max(0, Math.min(1, Number(event.target.value))) })} /></label></>}
    {action.type === "shader" && <><label>Target<input aria-label="Shader action target" value={action.target} onChange={(event) => setAction({ ...action, target: slugToken(event.target.value) })} /></label><label>Parameter<input aria-label="Shader action parameter" value={action.parameter} onChange={(event) => setAction({ ...action, parameter: slugToken(event.target.value) })} /></label><label>Value<input aria-label="Shader action value" value={String(action.value)} onChange={(event) => setAction({ ...action, value: parsePrimitive(event.target.value) as number | string | boolean })} /></label></>}
  </>;
}
...
function defaultAction(type: ActionType, graph: InteractionGraph): InteractionAction {
  ...
  if (type === "sequence") return { type, name: "reveal", command: "play" };
  if (type === "camera") return { type, name: "detail", command: "play" };
  if (type === "audio") return { type, name: "ambient", command: "play", volume: 1 };
  return { type: "shader", target: "hero", parameter: "intensity", value: 1 };
}
...
function normalizeTrigger(node: InteractionTriggerNode): InteractionTriggerNode {
  ...
  if (["click", "hover-enter", "hover-leave"].includes(node.event)) return { ...base, target: node.target ?? "feature" };
  if (["scene-enter", "scene-exit"].includes(node.event)) return { ...base, sceneId: node.sceneId ?? "arrival" };
  if (node.event === "custom") return { ...base, name: node.name ?? "interaction" };
