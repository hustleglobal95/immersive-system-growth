...
export interface HandoffWeights {
  outgoing: number;
  incoming: number;
  cover: number;
}
...
export function occlusionHandoff(t: number): HandoffWeights {
  ...
  return {
    outgoing: 1 - easeCinematic(clamp01((x - 0.34) / 0.22)),
    incoming: easeCinematic(clamp01((x - 0.44) / 0.22)),
    cover,
  };
}
...
export function modelExchange(t: number): HandoffWeights {
  const x = clamp01(t);
  return {
    outgoing: 1 - easeSmooth(clamp01((x - 0.4) / 0.2)),
    incoming: easeSmooth(clamp01((x - 0.4) / 0.2)),
    cover: 0,
  };
}
...
export function surfaceDive(t: number) {
  ...
  return {
    scale: 1 + x * 5,
    blur: x * 8,
    incoming: easeSmooth(clamp01((x - 0.68) / 0.24)),
  };
}
