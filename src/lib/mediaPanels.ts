...
export interface PanelWindow { start: number; end: number; enterStart: number; exitStart: number; first: boolean; last: boolean; direction: "up" | "down"; exitDirection?: "up" | "down"; zoom: number; transition?: MediaTransition; exitCut?: boolean }
...
export function sampleMediaPanel(p: number, w: PanelWindow, compact = false) {
  ...
  return {
    ...
    panelY: transition === "slide" ? sign * (1-enter)*100 - (w.exitCut ? 0 : exitSign*leave*35) : 0,
    imageY: isCut ? 0 : -sign*(1-enter)*travel + exitSign*leave*travel,
    scale: isCut ? 1
      : transition === "zoom"
