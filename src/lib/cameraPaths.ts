...
export function sampleCameraPath(from: Vec3, to: Vec3, t: number, preset: CameraPathPreset): Vec3 {
  ...
  switch (preset) {
    ...
    case "orbit":
      return add(base, [fullWave * 0.55, wave * 0.25, Math.cos(Math.PI * 2 * t) * 0.35]);
    case "crane":
