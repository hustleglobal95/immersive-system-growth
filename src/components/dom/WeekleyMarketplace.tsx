...
import { useWeekleySectionMotion } from "@/src/components/motion/useWeekleySectionMotion";
import { CinematicTextReveal, MagneticSurface } from "@/src/components/motion";
...
function CommunityGallery({ community, reducedMotion }: { community: Community; reducedMotion: boolean }) {
  const [photo, setPhoto] = useState(0);
  const image = useRef<HTMLImageElement>(null);
  const photos = [community.image, community.detailImage];
  useEffect(() => {
    if (!image.current || reducedMotion || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    const tween = gsap.fromTo(image.current, { clipPath: "inset(0% 100% 0% 0%)", scale: 1.025 }, {
      clipPath: "inset(0% 0% 0% 0%)", scale: 1, duration: 0.65, ease: "power3.inOut", clearProps: "clipPath,transform",
    });
    return () => { tween.revert(); };
  }, [photo, reducedMotion]);
  return <div className="dw-detail-gallery">
    <div className="dw-detail-gallery__stage"><img ref={image} className="dw-detail-hero" src={photos[photo]} alt={`${community.fullName}, community photograph ${photo + 1} of 2`} width="1200" height="800" /><span className="dw-detail-gallery__count">0{photo + 1} / 02</span></div>
    <div className="dw-detail-gallery__thumbnails" aria-label="Community photography">{photos.map((src, index) => <button key={src} aria-label={`Show community photograph ${index + 1}`} aria-pressed={photo === index} onClick={() => setPhoto(index)}><img src={src} alt="" width="96" height="64" /><span>{index === 0 ? "The community" : "A closer look"}</span></button>)}</div>
  </div>;
}
...
export function WeekleyMarketplace() {
  ...
  useWeekleySectionMotion(site, reducedMotion);
  useEffect(() => {
    if (!grid.current || reducedMotion || window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    gsap.registerPlugin(ScrollTrigger);
    const context = gsap.context(() => {
      const columns = window.innerWidth > 900 ? 3 : window.innerWidth > 600 ? 2 : 1;
      gsap.utils.toArray<HTMLElement>(".dw-market-card").forEach((card, index) => {
        gsap.timeline({ scrollTrigger: { trigger: card, start: "top 96%", once: true }, delay: (index % columns) * 0.065 })
          .fromTo(card, { y: 24, opacity: 0 }, { y: 0, opacity: 1, duration: 0.65, ease: "power3.out", clearProps: "transform,opacity" }, 0)
          .fromTo(card.querySelector(".dw-market-card__image"), { clipPath: "inset(0% 0% 100% 0%)" }, { clipPath: "inset(0% 0% 0% 0%)", duration: 0.8, ease: "power3.inOut", clearProps: "clipPath" }, 0);
      });
    }, grid);
  ...
  const activeFilters = [
    ...(market !== "All locations" ? [{ label: market, clear: () => setMarket("All locations") }] : []),
    ...(budget !== "Any price" ? [{ label: `Up to ${money(Number(budget))}`, clear: () => setBudget("Any price") }] : []),
    ...(beds !== "Any beds" ? [{ label: `${beds}+ bedrooms`, clear: () => setBeds("Any beds") }] : []),
    ...(homeType !== "All homes" ? [{ label: homeType === "Townhome" ? "Townhomes" : homeType, clear: () => setHomeType("All homes") }] : []),
  ...
  return <main className="dw-site" ref={site} id="top">
    ...
    <div id="main-content">
      ...
      <section className="dw-section dw-market-section" id="communities" aria-labelledby="markets-title">
        ...
        {!!activeFilters.length && <div className="dw-active-filters" aria-label="Active search filters">{activeFilters.map(({ label, clear }) => <button key={label} onClick={clear} aria-label={`Remove ${label} filter`}>{label}<Icon name="close" /></button>)}</div>}
