...
export function CinematicMedia() {
  ...
  useEffect(()=>{
    ...
    const tracks=panels.map(panel=>{
      ...
      let playing=false;
      return {panel,video,
        window:{start:scene.range[0],end:scene.range[1],enterStart:prior?scene.range[0]-(prior.range[1]-prior.range[0])*(scene.media?.overlap??.25):0,exitStart:next?scene.range[1]-(scene.range[1]-scene.range[0])*(next.media?.overlap??.25):1,first:index===0,last:index===experience.scenes.length-1,direction:scene.media?.direction??"up" as const,exitDirection:next?.media?.direction??"up" as const,zoom:scene.media?.zoom??1.06},
    ...
    const render=(p:number)=>{
      for(const t of tracks){const state=sampleMediaPanel(p,t.window,compact.matches);t.panel.style.visibility=state.visible?"visible":"hidden";t.panelY(state.panelY);t.imageY(state.imageY);t.scale(state.scale);t.playback(state.visible);}
    };
  ...
  return <div ref={root} className="cinematic-media" aria-hidden="true">
    {experience.scenes.map((scene,index)=>{
      ...
      return <div key={`${scene.id}-${preview}`} className="media-panel" data-media-panel={index} style={{zIndex:index}}>
        <div className="media-panel__inner" style={{"--media-position":`${media?.position[0]??50}% ${media?.position[1]??50}%`,"--media-position-mobile":`${media?.mobilePosition[0]??50}% ${media?.mobilePosition[1]??50}%`} as CSSProperties}>
