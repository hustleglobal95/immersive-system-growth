...
export function CinematicSystemsLayer(){
  ...
  useEffect(()=>{
    ...
    if(!media)return;
    const previous={transform:media.style.transform,maskImage:media.style.maskImage,webkitMaskImage:media.style.webkitMaskImage,filter:media.style.filter};
    ...
    return()=>{media.style.transform=previous.transform;media.style.maskImage=previous.maskImage;media.style.webkitMaskImage=previous.webkitMaskImage;media.style.filter=previous.filter;};
  },[activeScene,composed,config,reduced]);
  ...
  useEffect(()=>{
    ...
    for(const item of composed?.procedural??[]){const c=item.config;ctx.save();ctx.globalAlpha=item.opacity;ctx.strokeStyle=c.color;ctx.fillStyle=c.color;ctx.globalCompositeOperation=c.blendMode==="normal"?"source-over":c.blendMode==="screen"?"screen":c.blendMode==="multiply"?"multiply":"overlay";
      if(c.kind==="contours"){ctx.lineWidth=.8;for(const path of generateContourPaths(c,width,height))ctx.stroke(new Path2D(path));}
      if(c.kind==="halftone"){const limit=quality==="low"?1200:quality==="medium"?4000:c.count;for(const point of generateHalftonePoints(c,width,height).slice(0,limit)){const influence=c.interactive?Math.max(0,1-Math.hypot(point.x-width*(pointer.x*.5+.5),point.y-height*(-pointer.y*.5+.5))/260):0;ctx.beginPath();ctx.arc(point.x,point.y,point.radius*(1+influence*c.response),0,Math.PI*2);ctx.fill();}}
      ...
      if(c.kind==="grid"){ctx.lineWidth=.6;for(let x=0;x<=c.columns;x++){const px=x/c.columns*width;ctx.beginPath();ctx.moveTo(px,0);ctx.lineTo(px+pointer.x*c.distortion*20,height);ctx.stroke();}for(let y=0;y<=c.rows;y++){const py=y/c.rows*height;ctx.beginPath();ctx.moveTo(0,py);ctx.lineTo(width,py-pointer.y*c.distortion*20);ctx.stroke();}}
      ctx.restore();}
