...
Studio Pro tests additionally cover dynamic runtime configuration, art-direction defaults, duplicate transition-layer IDs, deterministic layer envelopes, asset path traversal, real Sharp image output, atomic manifest registration and mocked GitHub review-PR requests. Motion tests cover exact endpoints, cubic curves, reverse seeking, target bounds, duplicate targets, mapped GLB nodes, transition-layer references and desktop/mobile precedence. Browser coverage applies a template, edits scene direction, composes a layer, authors motion with undo/redo and mounts the production preview.
