...
It contains **106 public studio case studies, technical breakdowns and source studies** from Dogstudio, Lusion, makemepulse, Bonhomme, 14islands and Codrops.

Combined with the 49-template GetLayers corpus, Forge now has **155 immersive references** and **84 executable construction patterns**.

## What this corpus is for

The goal is to learn recurring construction logic:

- how studios stage one dominant subject;
- how they split semantic DOM and spatial rendering;
- how they keep long experiences continuous;
- how they author camera paths;
- how they connect interaction to a brand or story idea;
- how they reduce expensive rendering without reducing the concept;
- how they make transitions serve both narrative and resource scheduling;
- how they preserve the same experience on mobile through fidelity changes or medium substitution.

The goal is **not** to copy a studio's exact visual identity, proprietary engine or source code.

## Studios and references

### Dogstudio

1. **Virgin Galactic** — WebGL subscenes, planned inter-scene transitions, hotspots/media cards, DOM/WebGL event architecture, precomputed environment-map reflections.
2. **Zoox** — realtime WebGL chosen as a fidelity/asset-weight tradeoff; tight 3D artist/runtime collaboration.
3. **Geske** — product storytelling mixed with technical proof; polygon reduction, compression, lazy/adaptive loading; shaders used to explain product relationships.
4. **DEPT Pioneer** — guided conversational route and direct content route coexist.
5. **Meebits** — one ON/OFF interaction transforms the character and interface; one world grammar spans type, iconography, motion and sound.
6. **Tomorrowland Around the World** — one world supports day/night states; environmental details sell place; same concept spans mobile/desktop.
7. **Adobe Taking Shape** — flat concepts were rejected when they failed spatial camera translation; clean WebGL architecture allowed late additions.
8. **KIKK Festival 2018** — playful organisms contrasted with rigorous scientific-grid language across image, hover, transition and canvas behavior.

### Lusion

9. **Devin AI** — advanced product story uses subtle animation/interaction instead of maximal spectacle.
10. **Zero Tech** — realtime 3D and scroll navigation are one system.
11. **Max Mara: Bearing Gifts** — recurring character continuity; cross-device planning; non-realtime lighting for consistent performance.
12. **Of The Oak** — Houdini scene data converted into a 3.5 MB runtime format; repeated structures use instancing.
13. **Worldcoin Globe** — dense country/dot information encoded into image data for GPU reconstruction; live APIs drive visual state.
14. **The Turn of the Screw Trailer** — physical-camera-matched splines, camera-corridor geometry culling, hover drives both light and audio focus.
15. **ION / X1** — modeling, texturing, shaders, lighting and UI are treated as one product art direction.
16. **My Little Storybook** — hand-drawn 2D intentionally coexists with 3D WebGL rather than chasing realism.

### makemepulse

17. **Riot Anima Squad** — enemy exploration teaches tactics/traits; easter eggs reward curiosity after the core content works.
18. **Audemars Piguet: The House of Wonders** — three spatial environments correspond to three narrative stages; desktop/mobile/VR preserve one story structure.
19. **Weleda: The Open Garden** — mobile-first enhanced 360 environments used instead of a heavier free-roaming 3D world.
20. **Nasdaq 50th Anniversary** — archival content becomes an interactive chronological exhibition.
21. **La Prairie: The Pursuit of Harmony** — exhibition, product and science occupy separate rooms inside one Bauhaus-derived visual system.
22. **A World With No Heroes** — one persistent real-time world contains 50 cameras, systemic events and day/night state.

### Bonhomme

23. **Black Messiah** — changing a word changes the meaning of a sentence; interaction affects narrative meaning instead of only appearance.
24. **TAG Heuer Skipper** — immersive sailing world and rational product panels are separate modes connected by intentional transitions; mobile-first scene budgets.
25. **TAG Heuer Carrera 60th** — changing product camera angles are chapters in the watch/history narrative rather than free orbit.
26. **Bonhomme 10 Years** — WebGL complexity is intentionally hidden behind a naive illustrated universe; visible technology is not the aesthetic goal.

### 14islands

27. **Hatom / Syfy + USH** — different spatial metaphors are chosen for adjacent products: cubes for entity profiles, cosmic continuous travel for feature explanation.

### Codrops technical case studies

28. **ZERO** — drawing a zero is both the entry gesture and the cause of the first shader reveal; semantic gesture validation beats raw path matching.
29. **Ten Years Away** — fixed R3F canvas, DOM captions above it, hard-authored chapter compositions, velocity-reactive shader atmosphere.
30. **Stefan Vitasović Portfolio 2025** — shared WebGL geometry, unused meshes skipped, mobile swaps WebGL for HTML5 video while preserving motion principles.
31. **Dash Creative: Magnetic Commerce** — interaction is derived directly from the brand thesis; the logo physically behaves like magnetism.
32. **Ciel Rose** — WebGL planes align with DOM media; crop/aspect ratio remains an authored CMS control.
...
This expansion first added 15 patterns on top of the original 29, then the second research pass added 8 more. The current library contains **84 patterns**:

1. **interaction-as-thesis**
2. **multi-subscene-transition-budget**
3. **context-layer-over-world**
4. **simulated-expensive-lighting**
5. **authored-camera-corridor**
6. **camera-corridor-culling**
7. **source-structure-to-runtime-format**
8. **data-drives-world-state**
9. **mixed-media-world**
10. **choose-medium-by-capability**
11. **timeline-as-exhibition**
12. **gesture-gates-world**
...
### 1. Interaction is strongest when it has semantic consequence

Black Messiah, Dash and ZERO all reinforce the same rule from different directions:

- the interaction changes meaning;
- the interaction behaves like the brand idea;
- or the interaction is itself the narrative threshold.

Forge should ask **what the visitor causes**, not merely what moves under the pointer.

### 2. High fidelity often comes from substitution, not brute force

Virgin Galactic, Max Mara and Turn of the Screw all show versions of the same production idea:

- use precomputed environment maps instead of costly realtime reflection;
- bake or precompute lighting when the user does not benefit from solving it live;
- cull geometry against the actual camera corridor.

Forge should optimize perceived result, not defend expensive implementation for its own sake.

### 3. The camera is a design constraint

Turn of the Screw and Carrera demonstrate that a camera path can be the page structure itself.

Once the allowed camera corridor is known, Forge can:

- art-direct every reachable frame;
- remove geometry the camera can never see;
- choose focal/lens behavior intentionally;
- adapt a separate portrait corridor for mobile.

### 4. Mobile can keep the concept while changing the medium

Stefan Vitasović swaps desktop WebGL for HTML5 video on mobile while preserving interaction/motion language. Weleda uses enhanced 360 environments for a mobile-first spatial experience.

Forge should not interpret "preserve the concept" as "run identical code everywhere."

### 5. A transition is also resource choreography

Virgin Galactic's subscenes and Weleda's linked 360 environments reinforce that the scene boundary is where:

- next assets must already be ready;
- previous assets can be released;
- visual overlap hides the swap;
- the carried anchor tells the visitor that continuity remains intact.

### 6. Authoring formats should not dictate runtime formats

Of The Oak and Worldcoin Globe both replace obvious object-by-object delivery with runtime-specific data representations.

Forge should be willing to:
...
- texture-encode;
- bake;
- or otherwise reconstruct authoring data for the GPU.

### 7. Immersive worlds need content architecture

Virgin Galactic, DEPT Pioneer and A World With No Heroes all use spatial experiences without trapping information inside the scene.
...
This is what keeps a spatial website usable rather than turning it into a demo.

### 8. Story structure comes before technology

The Spark, Podium, Audemars Piguet and La Prairie all organize the experience around narrative stages before deciding where 3D belongs.

Forge should be able to build:

- a full persistent 3D world;
- a selective WebGL/DOM hybrid;
- an enhanced 360 journey;
- or an almost entirely editorial experience
...
Director now retrieves from the combined **155-reference corpus**, not the GetLayers set alone.
...
- a GetLayers visual composition;
- a Dogstudio scene-transition lesson;
- a Lusion asset-pipeline lesson;
- a Codrops technical motion pattern;
- a Bonhomme interaction principle.

The result should be a new client-specific experience, not a collage of copied executions.

## Sources

Every executable record stores its public source URL and evidence notes in:

`src/platform/director-intelligence/broaderReferenceCorpus.ts`

Use the source itself when deeper verification is needed. Do not upgrade a case-study observation into a technical implementation claim unless the source explicitly supports it.
