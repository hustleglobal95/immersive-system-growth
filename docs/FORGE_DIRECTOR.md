...
Reusable construction knowledge lives in `src/platform/director-intelligence/constructionKnowledge.ts` and is compiled into the existing Creative Plan. The current research pool contains **133 evidence-graded immersive references** and **78 executable construction patterns**, spanning composition, typography, camera direction, interaction causality, DOM/WebGL coordination, asset pipelines, mobile translation, transition budgeting and first-use performance.
