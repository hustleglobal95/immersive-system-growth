...
- `app/robots.ts` decides this. Answer-engine crawlers include `GPTBot`,
...
- Verify server-side rendering: answer-engine crawlers are far less reliable at
  ...
  to them. In this project, content is server-rendered and animations only
  animate — keep it that way.

## 2. `llms.txt`

A plain-markdown map of the site at `/llms.txt` for language models. Serve it
from `app/llms.txt/route.ts` (a Route Handler returning `text/plain`) so it
