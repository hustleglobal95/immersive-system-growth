...
**Canonical implementations already in this repository.** Do not invent new shapes; these are
the existing ones. Fuller worked code for patterns this repo has not needed yet is in
`references/patterns.md`, written against another workspace -- read it for the shape, not for
the paths.

| pattern | where it already lives here |
|---|---|
| device tiering, quality tiers, reduced motion | `src/runtime/SystemProfile.tsx`, `src/store/experienceStore.ts` (`quality`, `adaptiveTier`, `deviceCeiling`) |
| DPR and pixel budget | `config/experience.json` → `runtime.minDpr` / `maxDpr` / `maxPixels` |
| one shared scroll clock | `src/lib/cinematicProgress.ts` — every subscriber samples it; do not add a second |
| in-view + hidden-tab render gate | `src/components/dom/MediaShader.tsx` (IntersectionObserver + panel-opacity gate), `src/components/dom/ImageRoll.tsx` |
| deterministic sampler, no playback clock | `src/lib/mediaPanels.ts`, `src/lib/motionSequencer.ts` |
| shared easing across CPU and GPU | `src/lib/easing.ts` ↔ `src/lib/maskShader.ts` (the same quintic in both) |
| WebGL context loss and disposal | `src/components/dom/MediaShader.tsx` cleanup, `src/components/three/MaskedMediaLayer.tsx` |
| mask backend selection, WebGL vs CSS | `src/lib/maskReveal.ts` → `resolveMaskBackend` |
| the canvas itself | `src/components/three/SceneCanvas.tsx` — rule 1, one persistent canvas |
...
- **Measure a production build, never the dev server.** Dev invalidates §1 (the
  ...
  the apparent frame rate). `npm run build && npx next start` — and **kill the old
...
`CLAUDE.md` — check the casing in the vault you are actually in):
